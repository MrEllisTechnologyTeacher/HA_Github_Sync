...
class GitOps:
    ...
    def _run(
        ...
        if check and result.returncode != 0:
            ...
            # Surface actionable hints for common auth failures
            if "403" in stderr or "Write access to repository not granted" in stderr:
                stderr += (
                    "\nHint: your GitHub token may lack 'repo' write scope, "
                    "or may not have access to this repository. "
                    "Generate a new PAT at https://github.com/settings/tokens "
                    "with 'repo' (classic) or 'Contents: Read and write' (fine-grained) permissions."
                )
    ...
    def set_gitignore(self, patterns: List[str]) -> None:
